/*
  ...
  - You are about to alter the column `salary` on the `Job` table. The data in that column could be lost. The data in that column will be cast from `DoublePrecision` to `Decimal(65,30)`.
  ...
  - Added the required column `address` to the `ApplicantProfile` table without a default value. This is not possible if the table is not empty.
  - Added the required column `locationId` to the `ApplicantProfile` table without a default value. This is not possible if the table is not empty.
  ...
  - Added the required column `address` to the `CompanyProfile` table without a default value. This is not possible if the table is not empty.
  - Added the required column `locationId` to the `CompanyProfile` table without a default value. This is not possible if the table is not empty.
  ...
  - Added the required column `isInformationCompleted` to the `User` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE "ApplicantEducation" ALTER COLUMN "stillAttending" DROP DEFAULT,
ALTER COLUMN "version" DROP DEFAULT;

-- AlterTable
ALTER TABLE "ApplicantExperience" ALTER COLUMN "currentlyWorking" DROP DEFAULT,
ALTER COLUMN "version" DROP DEFAULT;
...
DROP COLUMN "role",
ADD COLUMN     "isInformationCompleted" BOOLEAN NOT NULL,
...
-- AddForeignKey
ALTER TABLE "ApplicantProfile" ADD CONSTRAINT "ApplicantProfile_locationId_fkey" FOREIGN KEY ("locationId") REFERENCES "Location"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "Resume" ADD CONSTRAINT "Resume_userId_fkey" FOREIGN KEY ("userId") REFERENCES "User"("id") ON DELETE RESTRICT ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE "CompanyProfile" ADD CONSTRAINT "CompanyProfile_locationId_fkey" FOREIGN KEY ("locationId") REFERENCES "Location"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
